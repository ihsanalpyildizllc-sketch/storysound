...
exports.handler = async (event) => {
  ...
  const { jobId, names, story, genre, mood, forWhom } = body;
  ...
  try {
    ...
    if (ld.error) {
      ...
      return { statusCode: 200, body: "Done with error" };
    }
    ...
    if (!audioPart) {
      await store.setJSON(jobId, { status: "error", error: "No audio returned", song_title: song.song_title, lyrics: song.lyrics, parts_count: parts.length });
    ...
    return { statusCode: 200, body: "Song generated successfully" };
