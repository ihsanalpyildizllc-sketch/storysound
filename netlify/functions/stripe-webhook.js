...
exports.handler = async (event) => {
  ...
  try {
    ...
    await postmark.sendEmail({
      ...
      HtmlBody: `<div style="font-family:Georgia,serif;max-width:560px;margin:0 auto;padding:32px 24px">
<h1 style="font-size:28px;color:#1A1614">Your song is here, ${firstName} 🎵</h1>
<p style="color:#666;font-size:16px">We turned your love story into <strong>"${songData.song_title}"</strong> — ${meta.genre}, ${meta.mood.toLowerCase()} mood. The MP3 is attached to this email.</p>
<hr style="border:none;border-top:1px solid #eee;margin:24px 0">
<h2 style="font-size:18px;color:#1A1614">Lyrics</h2>
<pre style="font-family:Georgia,serif;font-size:15px;color:#333;line-height:1.9;white-space:pre-wrap">${songData.lyrics}</pre>
<hr style="border:none;border-top:1px solid #eee;margin:24px 0">
<p style="color:#999;font-size:13px">Not in love with your song? Reply to this email and we'll revise it for free. — StorySound</p>
</div>`,
    ...
    console.log('✅ Delivered:', meta.email, songData.song_title);
    ...
    try {
      const { ServerClient } = require('postmark');
      const postmark = new ServerClient(process.env.POSTMARK_SERVER_TOKEN);
      await postmark.sendEmail({ From: process.env.FROM_EMAIL||'songs@storysound.ai', To: meta?.email, Subject: 'Your StorySound song is being composed 🎵', TextBody: `Hi ${meta?.names?.split('&')?.[0]?.trim()||'there'},\n\nYour song is being composed right now — you'll receive it within 15 minutes. Reply here with any questions.\n\n— StorySound` });
    } catch(e){}
