...
const { sendEmail } = require("./_shared");
  ...
  if (isUnlockOrder) {
    ...
    if (origOrderId && REDIS_URL && REDIS_TOKEN) {
      ...
      const lyricsBought = hasVariant(V_LYRICS) || attrs["Lyrics_Addon"] === "Yes" || lineItems.some(i => (i.title||"").toLowerCase().includes("lyric"));
      ...
      if (meta.email && songRec && songRec.status === "done") {
  ...
  const country = order.billing_address?.country_code || order.shipping_address?.country_code || null;
  const items = lineItems.map(i => i.title + " ($" + i.price + ")").join(", ");
  const titleHas = (s) => lineItems.some(i => (i.title || "").toLowerCase().includes(s));
  const lyricsBought = lineItems.some(i => ["44263011287129","44258586886233"].includes(String(i.variant_id))) || titleHas("lyric");
  const dlBought = lineItems.some(i => String(i.variant_id) === "44339845791833") || titleHas("download");
  const v3Bought = lineItems.some(i => String(i.variant_id) === "44263046381657") || titleHas("verse");
  const meta = {
    source: "create", paid: true, unlocked: true,
    ...
    priority: titleHas("priority"),
    forever: titleHas("forever memory"),
    vault: titleHas("vault"),
